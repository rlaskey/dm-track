...
foreach ($numbers AS $k => $v)
{
	...
	else $displayDay = $currentDay = $thisDay;

	$method = is_numeric($v->value) ? 'glucose' : 'insulin';
	echo '<tr class="linky" id="',$method,'-',$v->PK,'">',
		'<td>',$displayDay,'</td>',

		'<td>',
		anchor('input/'.$method.'/'.$v->PK,date('g:ia',$v->time)),
		'</td>',

		'<td',highlight_glucose($v->value),'>',$v->value,'</td>',
		'<td>',htmlentities($v->notes),'</td>',
		'</tr>',PHP_EOL;
